...
export function EventIntelligence() {
  ...
  const canRouteToAdjustment =
    routing?.outcome === "Governed forecast adjustment" && (residual?.applied ?? 0) !== 0;
  const alreadyRequested = selected
  ...
  const selectedRequest = selected
    ? adjustmentRequests.find((r) => r.originId === selected.id)
    : undefined;
  const applyBlockReason = (() => {
    ...
    if (residual.applied === 0)
      return "There is no residual impact to apply after the double-counting check.";
    return "";
  })();
  ...
  const counts = useMemo(() => {
    ...
    return {
      ...
      doubleCount: intelEvents.filter(
        (e) => e.reflection === "Fully reflected" || e.reflection === "Partially reflected",
      ).length,
    };
  }, [intelEvents]);
  ...
  const submitForm = () => {
    ...
    addIntelEvent({
      ...
      reflection: "Unclear",
      sourceChecks: evidenceSourcesChecked.map((source) => ({
        source,
        signal: "No signal" as const,
        reflectedPct: 0,
      })),
    });
    setShowForm(false);
  ...
  return (
    <div className="space-y-5">
        ...
        actions={
          <>
            ...
            >
              ...
              {showForm ? "Close form" : "Register event"}
            </button>
            {stageDone.events ? (
              <button
                type="button"
      ...
      <PrototypeNote>
        Illustrative prototype data. Events, evidence links and reflected-impact signals are seeded
        for demonstration.
      </PrototypeNote>

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <KpiTile
          label="Events in registry"
        ...
        <KpiTile
          ...
          delta="Planner approved"
          deltaTone="positive"
        />
        <KpiTile
          label="In review / recommended"
          value={String(counts.review)}
          delta="Awaiting decision"
          deltaTone="warning"
        />
        <KpiTile
          label="Watchlist & draft"
          value={String(counts.watchlist)}
          delta="No forecast change"
          deltaTone="neutral"
        />
        <KpiTile
          label="Applied to review"
          value={String(eventAdjustmentRequests.length)}
          delta="Explicitly selected"
          deltaTone="positive"
        />
      </div>
      ...
      {showForm && (
        <Panel
          title="Register a new event"
          description="New events enter the registry as Draft and must pass qualification before routing."
        >
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            <label className="block md:col-span-2">
              ...
              <input
                ...
                placeholder="e.g. OEM new-model launch — Q4 platform"
            ...
            <label className="block">
              ...
              <input
                value={form.plantScope}
                onChange={(e) => setForm({ ...form, plantScope: e.target.value })}
                className="mt-1 h-8 w-full rounded-md border border-input bg-surface px-2.5 text-xs"
              />
            </label>
            <label className="block">
            ...
            <label className="block">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, recurrence: e.target.value as IntelEvent["recurrence"] })
                }
                className="mt-1 h-8 w-full rounded-md border border-input bg-surface px-2 text-xs"
              >
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder="Document, e-mail, EDI schedule…"
                className="mt-1 h-8 w-full rounded-md border border-input bg-surface px-2.5 text-xs"
              />
            </label>
            <label className="block">
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, reliability: e.target.value as Reliability })}
                className="mt-1 h-8 w-full rounded-md border border-input bg-surface px-2 text-xs"
              >
            ...
            <label className="block">
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, probabilityPct: Number(e.target.value) })}
                className="num mt-1 h-8 w-full rounded-md border border-input bg-surface px-2.5 text-xs"
              />
            </label>
            <label className="block">
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, expectedImpact: Number(e.target.value) })}
                className="num mt-1 h-8 w-full rounded-md border border-input bg-surface px-2.5 text-xs"
              />
            </label>
            <label className="block">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, impactUnit: e.target.value as IntelEvent["impactUnit"] })
                }
                className="mt-1 h-8 w-full rounded-md border border-input bg-surface px-2 text-xs"
              >
                {impactUnits.map((u) => (
            ...
            <label className="block">
              ...
              <select
                value={form.pattern}
                onChange={(e) => setForm({ ...form, pattern: e.target.value as ImpactPattern })}
                className="mt-1 h-8 w-full rounded-md border border-input bg-surface px-2 text-xs"
              >
            ...
            <div className="flex items-end gap-2 md:col-span-3">
              ...
              <span className="text-[11px] text-muted-foreground">
                Evidence attachment is a placeholder in this prototype.
              </span>
            </div>
          </div>
        </Panel>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="mt-1.5 flex flex-wrap gap-1.5">
                        ...
                        <StatusPill tone={reflectionTone[event.reflection]}>
                          {event.reflection}
                        </StatusPill>
                    ...
                    <div className="shrink-0 text-right">
                      <p
                        className={cn(
                          ...
                          event.expectedImpact >= 0 ? "text-positive" : "text-risk",
                        )}
                      >
        ...
        {selected && routing && residual && (
          <div className="space-y-4">
            ...
            >
              ...
              {activeTab === "decision" && (
                <div
                  id="guide-event-decision"
                  tabIndex={-1}
                  className="mt-4 scroll-mt-52 space-y-3 outline-none"
                >
                  <div className="grid grid-cols-1 gap-3 rounded-md border border-border bg-surface-muted p-3 md:grid-cols-3">
                    ...
                    <label className="block">
                      ...
                      <div className="mt-1 flex items-center gap-1">
                        ...
                        />
                        <span className="text-[11px] text-muted-foreground">
                          {selected.impactUnit === "Percentage" ? "%" : "u"}
                        </span>
                      </div>
                    </label>
                    <div className="block">
                      ...
                      <div className="mt-1 flex h-8 items-center">
                        <StatusPill tone={routingTone[routing.outcome]}>
                          {routing.outcome}
                        </StatusPill>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 rounded-md border border-border p-3 md:grid-cols-[minmax(0,1fr)_auto]">
                    <div>
                      ...
                      <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">
                        Applying submits this event's residual impact to Forecast Review. All other
                        registry events stay unchanged.
                      </p>
                      {selectedRequest && (
                        <p className="mt-2 text-[11px] font-medium text-positive">
                          Submitted: {selectedRequest.title} ·{" "}
                          {selectedRequest.requestedImpactPct > 0 ? "+" : ""}
                          {selectedRequest.requestedImpactPct}%
                        </p>
                      )}
            ...
            {activeTab === "evidence" && (
              <>
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  <Panel
                    title="Event qualification checklist"
                    description="All six questions should be satisfied before an event routes into the official forecast."
                  >
                    ...
                    <p className="mt-3 text-xs text-muted-foreground">
                      ...
                      <span className="num font-semibold text-foreground">
                        {qualificationScore(selected.qualification)}/6
                      </span>
                    </p>
                  </Panel>
                          ...
                          {routingRules.map((r) => (
                            <tr
                              ...
                              className={cn(
                                ...
                                r.outcome === routing.outcome && "bg-accent",
                              )}
                            >
                ...
                <Panel
                  ...
                  actions={
                    <StatusPill tone={reflectionTone[selected.reflection]}>
                      {selected.reflection}
                    </StatusPill>
                  }
                >
                  <div className="flex flex-wrap gap-2">
                    {reflectionStates.map((state) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          updateIntelEvent(selected.id, { reflection: state as ReflectionState })
                        }
                        className={cn(
                          "rounded-md border px-2.5 py-1 text-[11px] font-medium",
                          selected.reflection === state
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
                    ...
                    <div className="rounded-md border border-border bg-surface-muted p-3">
                      ...
                      <div className="mt-2 space-y-1">
                        <MetricRow
                          label="Expected event impact"
                          value={`${residual.expected > 0 ? "+" : ""}${residual.expected}%`}
                        />
                        <MetricRow
                          label="Impact already reflected"
                          value={`${residual.alreadyReflected}%`}
                        />
                        <MetricRow
                        ...
                        />
                        <MetricRow
                          label="Reflected share of sources"
                          value={`${reflectedShare(selected)}%`}
                        />
                        <MetricRow
            ...
            {activeTab === "impact" && (
                ...
                actions={
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updateIntelEvent(selected.id, {
                        pattern,
                        curve: patternCurve(pattern, selected.expectedImpact),
                      });
                    }}
                    className="h-7 rounded-md border border-input bg-surface px-1.5 text-[11px]"
                  >
              ...
              >
                    ...
                    <BarChart data={curveData} margin={{ top: 6, right: 8, bottom: 0, left: 0 }}>
                      ...
                      <Bar dataKey="value" name="Impact" radius={[2, 2, 0, 0]}>
                        {curveData.map((d, i) => (
                          <Cell
                            key={i}
                            fill={d.value >= 0 ? "var(--color-accent-blue)" : "var(--color-risk)"}
                          />
                        ))}
                      </Bar>
                    </BarChart>
                ...
                <p className="mt-2 text-[11px] text-muted-foreground">
                  The curve is editable — a flat percentage is rarely the right shape for shutdowns,
                  ramps or advance buying.
                </p>
              </Panel>
            )}
          </div>
      ...
      {activeTab === "requests" && (
        <Panel
          title="Forecast-adjustment requests raised from events and scenarios"
          description="Approve or reject each request here, or from the same table on Forecast Review. Approving a request does not publish anything by itself — it still needs to be reflected in the forecast-review approval queue."
          bodyClassName="p-0"
        >
          <div className="overflow-x-auto">
                ...
                {adjustmentRequests.length === 0 && (
                  <tr>
                    <td colSpan={8} className="px-4 py-6 text-center text-muted-foreground">
                      No adjustment requests raised yet.
                    </td>
                  </tr>
                )}
              </tbody>
