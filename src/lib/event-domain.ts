...
export const seedIntelEvents: IntelEvent[] = [
  {
    id: "ie-0",
    name: "Apex Motors shutdown moved from September to October",
    category: "Confirmed calendar event",
    description:
      "Apex Motors confirmed that the annual plant shutdown moves from September to October 2026. History teaches the model a September dip that will not occur, and the October trough is invisible to the baseline. Open orders already carry part of the October reduction, so only the residual is applied.",
    customer: "Apex Motors (OEM)",
    skuScope: "CLT-1048 · Clutch Friction Assembly",
    plantScope: "North Plant — Coimbatore",
    startDate: "2026-09-01",
    endDate: "2026-11-30",
    recurrence: "One-time",
    evidenceSource: "Apex OEM schedule revision R-14 + signed shutdown calendar",
    evidenceLink: "attachment://apex-shutdown-calendar-fy27.pdf",
    reliability: "Confirmed document",
    probabilityPct: 97,
    expectedImpact: -38,
    impactUnit: "Percentage",
    pattern: "Multi-period demand transfer",
    curve: [0, 0, 82, -38, 14, 0],
    owner: "Customer account team · Apex",
    status: "Recommended",
    qualification: q(true, true, true, true, true, true),
    reflection: "Partially reflected",
    sourceChecks: checks([
      ["Open orders", "Clear signal", 14],
      ["OEM/customer schedules", "Clear signal", 12],
      ["Backlog", "Weak signal", 3],
      ["Purchase orders", "Weak signal", 2],
      ["Recent demand", "No signal", 0],
      ["Existing model features", "Weak signal", 2],
      ["Previous adjustments", "No signal", 0],
      ["Inventory movements", "No signal", 0],
      ["Related SKU movement", "Weak signal", 1],
    ]),
    createdAt: "2026-07-14 09:05",
    modifiedAt: "2026-07-24 10:22",
  },
  {
    id: "ie-12",
    ...
    endDate: "2026-11-30",
    recurrence: "One-time",
    evidenceSource: "Apex production plan v7",
    evidenceLink: "attachment://apex-production-plan-v7.xlsx",
    reliability: "Direct customer input",
    probabilityPct: 88,
    expectedImpact: 14,
    impactUnit: "Percentage",
    pattern: "One-time spike",
    curve: [0, 0, 0, 0, 14, 0],
    owner: "Customer account team · Apex",
    status: "Recommended",
    qualification: q(true, true, true, true, true, true),
...
export const seedScenarioSpecs: ScenarioSpec[] = [
  {
    ...
    notes:
      "Apex recovers the October shutdown volume faster than planned. Shown alongside the official forecast for comparison only — it is never the approved operational forecast.",
    assumptions: [
      "November catch-up build 9% above the approved event-aware forecast",
      "December carries a further 6% of pull-forward",
      "October shutdown length unchanged",
    ],
    linkedEventIds: ["ie-0", "ie-12"],
    monthlyImpactPct: [2, 2, 2, 0, 9, 6],
    capacityCapPct: 100,
    promoted: false,
  ...
  {
    id: "ss-4",
    name: "Apex shutdown extends into November",
    type: "Worst case",
