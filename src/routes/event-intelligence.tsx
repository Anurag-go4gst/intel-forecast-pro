...
function EventIntelligence() {
  ...
  return (
    <div className="space-y-5">
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(["Watchlist", "Under review", "Recommended", "Approved", "Rejected", "Expired"] as EventStatus[]).map((s) => (
                  ...
                  >
                    {s === "Rejected" ? (
                      <span className="inline-flex items-center gap-1"><X className="h-3 w-3" aria-hidden /> Reject</span>
                    ) : (
                      `Set ${s}`
                    )}
                  </button>
                ...
                <div className="rounded-md border border-border bg-surface-muted p-3">
                  ...
                  {!canRouteToAdjustment && !alreadyRequested && (
                    <p className="mt-1.5 text-[11px] text-muted-foreground">
                      Blocked: routing outcome is “{routing.outcome}”, so no adjustment can be applied.
                    </p>
                  )}
                </div>
              </div>
      ...
      <Panel title="Forecast-adjustment requests raised from events and scenarios" description="Requests require approval in Forecast Review; nothing here changes the approved forecast on its own." bodyClassName="p-0">
        <div className="overflow-x-auto">
