...
import { approvalFromScenario } from "@/lib/governance-domain";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "What-if Scenarios — Demand Intelligence Platform" },
      {
        ...
        content:
          "Sandbox scenarios compared against the baseline and approved forecast. Nothing changes the official forecast automatically.",
      },
    ],
  }),
...
const typePresets: Record<ScenarioType, { curve: number[]; cap: number; assumptions: string[] }> = {
  "Best case": {
    ...
    assumptions: [
      "Market demand +6%",
      "All approved events land on plan",
  ...
  "Base case": {
    curve: [0, 0, 0, 0, 0, 0],
    cap: 100,
    assumptions: ["Approved events at residual impact only", "No further market movement"],
  },
  "Worst case": {
    curve: [-2, -5, -8, -10, -11, -12],
    cap: 92,
    assumptions: ["Market demand -8%", "Two OEM schedule cuts", "Capacity at 92%"],
  },
  "Event delayed": {
    curve: patternCurve("Delayed impact", 14),
    cap: 100,
    assumptions: ["Key event shifts two months later", "Peak impact unchanged"],
  },
  "Event cancelled": {
    curve: [0, 0, 0, 0, 0, 0],
    cap: 100,
    assumptions: ["Event removed from the plan", "Baseline restored for affected scope"],
  },
  "Higher ramp-up": {
    curve: patternCurve("Gradual ramp-up", 24),
    cap: 105,
    assumptions: ["Ramp accelerated by one month", "Peak impact raised to 24%"],
  },
  "Lower ramp-up": {
    curve: patternCurve("Gradual ramp-up", 9),
    cap: 100,
    assumptions: ["Ramp slower than nominated volume", "Peak impact reduced to 9%"],
  },
  "Demand shock": {
    curve: patternCurve("One-time spike", 26),
    cap: 100,
    assumptions: ["Single-period surge of 26%", "No lasting baseline change"],
  },
  "Supply-constrained case": {
    curve: [0, -2, -5, -6, -4, -2],
    cap: 82,
    assumptions: ["Capacity capped at 82%", "Lead time +12 days"],
  },
  "Custom scenario": {
    curve: [0, 1, 2, 3, 3, 3],
    cap: 100,
    assumptions: ["Analyst-defined assumptions"],
  },
};
...
function WhatIfScenarios() {
  ...
  const approvedCurve = useMemo(() => {
    ...
    intelEvents
      .filter(
        (e) =>
          e.status === "Approved" &&
          routeEvent(e).outcome !== "Explanation only — no additional adjustment",
      )
      .forEach((e) => {
  ...
  const implications = (spec: ScenarioSpec) => {
    ...
    const serviceLevel = Math.max(
      84,
      Math.min(99.2, 97.5 - (stockoutUnits / (approvedTotal || 1)) * 220),
    );
    return {
  ...
  const alreadyPromoted = selected
    ? adjustmentRequests.some((r) => r.originId === selected.id)
    : false;
  ...
  return (
    <div className="space-y-5">
        ...
        actions={
          <>
            <StatusPill tone="info">
              <FlaskConical className="h-3 w-3" aria-hidden /> Sandbox — official forecast never
      ...
      <PrototypeNote>
        Illustrative prototype data. Scenario impacts, capacity ceilings and service-level effects
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiTile
          ...
          unit="units"
          delta={formatSigned(((approvedTotal - baselineTotal) / (baselineTotal || 1)) * 100)}
          deltaTone="positive"
        />
        <KpiTile
          label="Scenarios saved"
          value={String(scenarioSpecs.length)}
          delta={`${compared.length} on chart`}
          deltaTone="info"
        />
        <KpiTile
      ...
      <div
        ...
        tabIndex={-1}
        className="grid scroll-mt-52 grid-cols-1 gap-4 outline-none xl:grid-cols-[minmax(0,360px)_minmax(0,1fr)]"
      >
        <div className="space-y-4">
          <Panel
            title="Create a scenario"
            description="Pick a scenario type to start from a preset monthly impact curve."
          >
            <div className="space-y-3">
              <label className="block">
                ...
                <input
                  ...
                  placeholder="e.g. Worst case — Q4 OEM cuts"
              ...
              <label className="block">
                ...
                <select
                  value={newType}
                  onChange={(e) => setNewType(e.target.value as ScenarioType)}
              ...
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
              >
          ...
          <Panel
            ...
            description="Tick a scenario to compare it on the chart."
            bodyClassName="p-0"
          >
            <ul className="divide-y divide-border">
              {scenarioSpecs.map((spec, index) => (
                <li
                  key={spec.id}
                  className={cn("px-4 py-3", spec.id === selected?.id && "bg-accent")}
                >
                  <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-2">
                    ...
                    >
                      ...
                      <div className="mt-1 flex flex-wrap gap-1.5">
                        {spec.promoted && (
                          <StatusPill tone="warning">Promoted for review</StatusPill>
                        )}
                    ...
                    <button
                      type="button"
                      onClick={() => cloneScenarioSpec(spec.id)}
                      className="shrink-0 rounded-md border border-input px-2 py-1 text-[11px] hover:bg-accent"
        ...
        <div className="space-y-4">
          <Panel
            title="Baseline, approved forecast and selected scenarios"
            description="Scenario lines are derived from the approved forecast and the scenario's monthly impact curve."
          >
                ...
                <LineChart data={chartData} margin={{ top: 6, right: 8, bottom: 0, left: 0 }}>
                  ...
                  {compared.map((spec, i) => (
                    <Line
                      ...
                      stroke={
                        scenarioColors[
                          scenarioSpecs.findIndex((s) => s.id === spec.id) % scenarioColors.length
                        ]
                      }
                      strokeWidth={2}
            ...
            <>
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  <div>
                    ...
                    <ul className="mt-1.5 space-y-1">
                      {selected.assumptions.map((a) => (
                        <li
                          key={a}
                          className="rounded-md border border-border bg-surface-muted px-2.5 py-1.5 text-xs"
                        >
                          {a}
                        </li>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => cloneScenarioSpec(selected.id)}
                      ...
                      <button
                        ...
                        onClick={() =>
                          updateScenarioSpec(selected.id, {
                            monthlyImpactPct: [...typePresets[selected.type].curve],
                            capacityCapPct: typePresets[selected.type].cap,
                          })
                        }
                        className="inline-flex items-center gap-1.5 rounded-md border border-input px-2.5 py-1.5 text-[11px] font-medium hover:bg-accent"
                      >
                      ...
                      <button
                        ...
                        onClick={() => {
                          const requestedImpactPct = Number(
                            (
                              selected.monthlyImpactPct.reduce((a, b) => a + b, 0) /
                              selected.monthlyImpactPct.length
                            ).toFixed(1),
                          );
                          promoteToReview(
                            {
                            ...
                            approvalFromScenario(selected, requestedImpactPct),
                          );
                        }}
                      ...
                      >
                        ...
                        {alreadyPromoted ? "Promoted for review" : "Promote for review"}
                      </button>
                    </div>
                    <p className="mt-1.5 text-[11px] leading-relaxed text-muted-foreground">
                      Promotion creates a forecast-adjustment request that requires approval. The
                      approved forecast is unchanged until then.
                    </p>
                  </div>
                </div>
              </Panel>
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <Panel
                  ...
                  description="Compared with the approved forecast over the six-month horizon."
                >
                  <div className="space-y-1">
                    <MetricRow
                      label="Scenario demand"
                      value={`${formatNumber(selectedImplications.total)} units`}
                    />
                    <MetricRow
                      label="Delta versus approved forecast"
                      value={formatSigned(selectedImplications.deltaPct)}
                      tone={selectedImplications.deltaPct >= 0 ? "positive" : "warning"}
                    />
                    <MetricRow
                      label="Stockout implication (unmet demand)"
                      value={`${formatNumber(selectedImplications.stockoutUnits)} units`}
                      tone={selectedImplications.stockoutUnits > 0 ? "risk" : "positive"}
                    />
                    <MetricRow
                      label="SKU-locations at stockout risk"
                      value={String(selectedImplications.skusAtRisk)}
                      tone={selectedImplications.skusAtRisk > 0 ? "risk" : undefined}
                    />
                    <MetricRow
                      label="Excess-inventory implication"
                      value={`${formatNumber(selectedImplications.excessUnits)} units`}
                      tone={selectedImplications.excessUnits > 0 ? "warning" : "positive"}
                    />
                    <MetricRow
                      label="SKU-locations at excess risk"
                      value={String(selectedImplications.skusExcess)}
                      tone={selectedImplications.skusExcess > 0 ? "warning" : undefined}
                    />
                    <MetricRow
                      label="Service-level implication"
                      value={`${selectedImplications.serviceLevel.toFixed(1)}%`}
                      tone={selectedImplications.serviceLevel < 95 ? "warning" : "positive"}
                    />
                  </div>
                </Panel>
                ...
                <Panel
                  ...
                  description="All scenarios ticked in the library."
                  bodyClassName="p-0"
                >
                            ...
                            <tr key={spec.id} className="border-b border-border last:border-0">
                              ...
                              <td
                                className={cn(
                                  ...
                                  imp.deltaPct >= 0 ? "text-positive" : "text-risk",
                                )}
                              >
                                {formatSigned(imp.deltaPct)}
                              </td>
                              <td className="num px-4 py-2 text-right">
                                {formatNumber(imp.stockoutUnits)}
                              </td>
                              <td className="num px-4 py-2 text-right">
                                {imp.serviceLevel.toFixed(1)}%
                              </td>
                            </tr>
                          );
                        })}
                        {compared.length === 0 && (
                          <tr>
                            <td colSpan={5} className="px-4 py-6 text-center text-muted-foreground">
                              Tick scenarios in the library to compare them.
                            </td>
                          </tr>
                        )}
                      </tbody>
      ...
      <Panel
        title="Promoted scenarios and event adjustments awaiting approval"
        description="Promotion raises a request only; the approved forecast changes after sign-off in Forecast Review."
        bodyClassName="p-0"
      >
        <div className="overflow-x-auto">
            ...
            <tbody>
              {adjustmentRequests.map((r) => (
                <tr key={r.id} className="border-b border-border last:border-0">
                  ...
                  <td className="num px-4 py-2 text-right">
                    {r.requestedImpactPct > 0 ? "+" : ""}
                    {r.requestedImpactPct}%
                  </td>
